...
static PINT_sm_action migration_test(
	struct PINT_smcb *smcb, job_status_s *js_p);

static struct PINT_state_s ST_migration_test;
static struct PINT_pjmp_tbl_s ST_migration_test_pjtbl[];
static struct PINT_tran_tbl_s ST_migration_test_trtbl[];
...
static struct PINT_tran_tbl_s ST_prelude_trtbl[] = {
	{ .return_value = 0 ,
	 .next_state = &ST_migration_test },
	{ .return_value = -1 ,
	 .next_state = &ST_send_negative_ack }
};

static struct PINT_state_s ST_migration_test = {
	 .state_name = "migration_test" ,
	 .parent_machine = &pvfs2_io_sm ,
	 .flag = SM_RUN ,
	 .action.func = migration_test ,
	 .pjtbl = NULL ,
	 .trtbl = ST_migration_test_trtbl 
};
 ...
 *           job_status_s* js_p
 *
 * Synopsis: Detect if an migration on this file
 *           
 */
static int migration_test(
			  struct PINT_smcb *smcb, job_status_s *js_p)
{
  struct PINT_server_op *s_op = PINT_sm_frame(smcb, PINT_FRAME_CURRENT);
  

  if ((mig_state == 1) && (s_op->req->u.io.handle == mig_datafile_handle)){
    js_p->error_code = -1;
    gossip_debug(GOSSIP_LB_DEBUG, "Acccess being migrated file: %lld \n", lld(mig_datafile_handle));
  }
