    ...
    PVFS_handle_extent_array *handle_extent_array)
{
    ...
    switch(layout->algorithm)
    {
		...
		/* diable rondom allocation starting I/O server */
		start_index = 0;
            }
...
*
*/
int PINT_is_io_server(struct server_configuration_s *config,
		      PVFS_fs_id fsid,
		      char* local_server)
{
    int ret = -PVFS_EINVAL;
    ...
    while ((cur_mapping = PINT_llist_head(tmp_server)))
    {
	    ...
	    //printf("hostalias=%s,local server=%s\n",cur_mapping->alias_mapping->host_alias,local_server);
	    
	    if(strcmp(cur_mapping->alias_mapping->host_alias,local_server) == 0) {
		return 1;
	    }
    }
    return 0;
...
/*PINT_cached_config_get_io_server_array
*
*/
int PINT_cached_config_get_io_server_array(
    struct server_configuration_s *config,
    PVFS_fs_id fsid,
    char* local_server,
    PVFS_BMI_addr_t *addr_array,
    int *inout_count_p)
{
    ...
    struct qlist_head *hash_link = NULL;
    struct config_fs_cache_s *cur_config_cache = NULL;
	 PVFS_BMI_addr_t local_bmi_addr = 0;
    PINT_llist *cur = NULL;
    ...
    //    printf("local server name:%s\n",local_server);

    /* TO fix, why do't need the config  */
    ret = cache_server_array(fsid);
    if (ret < 0)
    {
        return ret;
        ...
        while(cur)
        {
            ...
            if (strcmp(cur_alias->host_alias,local_server) == 0)
            {
	 BMI_addr_lookup(&local_bmi_addr,cur_alias->bmi_address);
		}
            cur = PINT_llist_next(cur);
	}	


        if (*inout_count_p < cur_config_cache->io_server_count -1)
        {
            return -PVFS_EMSGSIZE;
        }

        for(i = 0,j = 0; i < cur_config_cache->io_server_count; i++)
        {
				if(cur_config_cache->io_server_array[i].addr == local_bmi_addr){
				    //printf("addr-----------------%d,%d\n",cur_config_cache->io_server_array[i].addr,local_bmi_addr);
					continue;
				}
            addr_array[j] = cur_config_cache->io_server_array[i].addr;
				j++;
        }
