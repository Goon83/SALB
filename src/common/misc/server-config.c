...
/*xiuqiao:PINT_get_data_handle_by_host_alias
 *
 *returns pointer to PVFS_handle_extent_array refer to single host
 *alias
 *
 */
PVFS_handle_extent_array *PINT_get_data_handle_by_host_alias(
    struct server_configuration_s *config_s,
    struct filesystem_configuration_s *fs,
    char *alias)
{
    PVFS_handle_extent_array * ret = NULL;
    PINT_llist *cur = NULL;
    struct host_handle_mapping_s *cur_h_mapping = NULL;
        ...
        if (alias)
        {
            ...
            while(cur)
            {
                ...
                if (strcmp(cur_h_mapping->alias_mapping->host_alias,
                           alias) == 0)
                {
                    ret = &cur_h_mapping->handle_extent_array;
                    break;
                }
                cur = PINT_llist_next(cur);
            }
