...
/*xiuqiao:mask for set individual datafile handle*/
#define PVFS_ATTR_META_SINGLE_DFILE (1 << 12)
...
struct PVFS_single_dfile_s
{
	PVFS_handle new;
	PVFS_handle old;
};
typedef struct PVFS_single_dfile_s PVFS_single_dfile;
...
struct PVFS_metafile_attr_s
{
    ...
    PVFS_single_dfile metafile;  /*xiuqiao:for loadbanlance*/
};
...
#define encode_PVFS_metafile_attr_single_dfile(pptr,x) do{\
	encode_PVFS_handle(pptr,&(x)->metafile.new);	  \
	encode_PVFS_handle(pptr,&(x)->metafile.old);	  \
    }while(0)
#define decode_PVFS_metafile_attr_single_dfile(pptr,x) do{\
	decode_PVFS_handle(pptr,&(x)->metafile.new);	  \
	decode_PVFS_handle(pptr,&(x)->metafile.old);	  \
    }while(0)
#endif
...
#define encode_PVFS_object_attr(pptr,x) do { \
    ...
    if ((x)->mask & PVFS_ATTR_META_SINGLE_DFILE) \
	encode_PVFS_metafile_attr_single_dfile(pptr, &(x)->u.meta); \
...
#define decode_PVFS_object_attr(pptr,x) do { \
    ...
    if ((x)->mask & PVFS_ATTR_META_SINGLE_DFILE)		    \
	decode_PVFS_metafile_attr_single_dfile(pptr, &(x)->u.meta); \
    if ((x)->mask & PVFS_ATTR_DATA_SIZE) \
