...
enum PVFS_server_op
{
    ...
    PVFS_SERV_MIGRATION_POSTER = 39,
    PVFS_SERV_SET_DF_ATTR_POSTER = 40,
...
/*Goon: set data file attr request with metafile handle*/
struct PVFS_servreq_set_df_attr_poster
{
    PVFS_handle handle; /* handle of target datafile */
    PVFS_fs_id fs_id;   /* file system id */
    PVFS_size dfsize;   /* data file size is 0 */
    PVFS_handle metafile; 	/* metadatafile */
};
    
endecode_fields_4_struct(
    PVFS_servreq_set_df_attr_poster,
    PVFS_handle, handle,
    PVFS_fs_id, fs_id,
    PVFS_size, dfsize,
    PVFS_handle,metafile);

#define PINT_SERVREQ_SET_DF_ATTR_POSTER_FILL(__req,				   \
                                        __fs_id,                   \
                                        __handle,               \
					__dfsize,                 \
                                        __metafile)		   \
do {                                                               \
    memset(&(__req), 0, sizeof(__req));                            \
    (__req).op = PVFS_SERV_SET_DF_ATTR_POSTER;                   \
    (__req).u.set_df_attr_poster.fs_id = (__fs_id);              \
    (__req).u.set_df_attr_poster.handle = (__handle);\
    (__req).u.set_df_attr_poster.dfsize= (__dfsize);\
    (__req).u.set_df_attr_poster.metafile= (__metafile); \
} while (0);

struct PVFS_servresp_set_df_attr_poster
{
    PVFS_handle handle; /* handle of target object */    
};
endecode_fields_1_struct(
    PVFS_servresp_set_df_attr_poster,
    PVFS_handle, handle);



/*file migration request*/
struct PVFS_servreq_migposter
{
    PVFS_handle handle; /* handle of target object */
    PVFS_fs_id fs_id;   /* file system */
    PVFS_size dfsize;   /*data file size*/
    PVFS_handle metafile; 	/* metadatafile */
};
    
endecode_fields_4_struct(
    PVFS_servreq_migposter,
    PVFS_handle, handle,
    PVFS_fs_id, fs_id,
    PVFS_size, dfsize,
    PVFS_handle,metafile);

#define PINT_SERVREQ_MIGRATION_POSTER_FILL(__req,              \
                                        __fs_id,                   \
                                        __handle,               \
					__dfsize,                 \
                                        __metafile)		   \
do {                                                               \
    memset(&(__req), 0, sizeof(__req));                            \
    (__req).op = PVFS_SERV_MIGRATION_POSTER;                   \
    (__req).u.migposter.fs_id = (__fs_id);              \
    (__req).u.migposter.handle = (__handle);\
    (__req).u.migposter.dfsize= (__dfsize);\
    (__req).u.migposter.metafile= (__metafile); \
} while (0);

struct PVFS_servresp_migposter
{
    PVFS_handle handle; /* handle of target object */    
};
endecode_fields_1_struct(
    PVFS_servresp_migposter,
    PVFS_handle, handle);
/* server request *********************************************/
...
struct PVFS_server_req
{
	...
	struct PVFS_servreq_migposter migposter;	
	struct PVFS_servreq_set_df_attr_poster set_df_attr_poster;	
    } u;
};
#ifdef __PINT_REQPROTO_ENCODE_FUNCS_C
...
struct PVFS_server_resp
{
	...
	struct PVFS_servresp_migposter migposter;
	struct PVFS_servresp_set_df_attr_poster set_df_attr_poster;
    } u;
};
endecode_fields_2_struct(
    PVFS_server_resp,
