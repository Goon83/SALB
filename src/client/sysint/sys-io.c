...
/*
 * set to 1, when detect migration at server
 * set to 0, default
 */
int     migration_at_server_flag = 0;
int     migration_server_index;
PVFS_handle migration_server_handle;
...
# 264 "src/client/sysint/sys-io.sm"
  ...
  if (ret == IO_RECV_COMPLETED)
    {
      ...
      gossip_debug(GOSSIP_IO_DEBUG,
		   "\n\nClient: cur_ctx->msg.op_status = %d \n\n",
		   cur_ctx->msg.op_status);

      if (ret < 0 || (cur_ctx->msg.op_status == -1) )
        {
	  char buf[64] = {0};
	  ...
	  /*
	   * if recv failed, probably have to do the send again too
	   * but if detect migration at server , keep it
	   * and don't post flow
	   */
	  if ( cur_ctx->msg.op_status != -1){
	    cur_ctx->msg_send_has_been_posted = 0;
	    cur_ctx->msg_recv_has_been_posted = 0;
	  }
	    
	  /*
	   * if migration detect, set global flag 
	   */
	  if ( cur_ctx->msg.op_status == -1){
	    migration_at_server_flag = 1;
	    migration_server_index = cur_ctx->index;
	    migration_server_handle = cur_ctx->data_handle;
	  }
					 ...
					 struct PINT_smcb *smcb, job_status_s *js_p)
{
  ...
  /*
   *If migration detect, go to getattr immediatedly 
   */
  if(migration_at_server_flag){
    gossip_debug(GOSSIP_LB_DEBUG, "\n\n io_analyze_results-->>Detect server migration \n\n");
    
    //printf("\n io_analyze_results-->>Detect server migration: %d, %lld \n", migration_server_index, lld(migration_server_handle)); 

    for(index = 0; index < sm_p->getattr.attr.u.meta.dfile_count; index++){ 
      printf("#%d, %llu \n",index,sm_p->getattr.attr.u.meta.dfile_array[index]); 
    } 
    
    /*
     * Invalite local acache and ncache
     */
    PINT_acache_invalidate(sm_p->object_ref);

    migration_at_server_flag = 0;    
    js_p->error_code = IO_RETRY;
					 ...
					 struct PVFS_server_resp **resp)
{
  ...
  if (cur_ctx->msg.recv_status.error_code || (cur_ctx->msg.op_status != 0 && cur_ctx->msg.op_status != -1))
    {
      gossip_debug(
					  ...
					  PINT_client_io_ctx **out_ctx)
{
  ...
  *out_ctx = cur_ctx;
  ...
  //if ((resp->status == -1) ){
    //gossip_debug(GOSSIP_LB_DEBUG, "\n\n ++ resp->status=%d, resp-bs=%d ++\n\n", resp->status, resp->u.io.bstream_size);
  //}
